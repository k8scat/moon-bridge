...
Moon Bridge 是一个协议转换和模型路由代理。它对外暴露 **OpenAI Responses** 端点（`/v1/responses`，也支持无 `/v1` 的 `/responses`），背后可以接任意兼容 **Anthropic Messages** 协议的上游 Provider，也可以直通上游 OpenAI Responses Provider。客户端指定不同的模型别名时，它自动把请求路由到对应的上游 Provider，并在不同协议之间自动转换。
...
v5 配置将模型元数据、Provider 声明、路由规则解耦为三个顶层区块：

- **顶层 `models`**：共享模型元数据（context_window、extensions、reasoning 等级等），以 slug 为 key
- **顶层 `providers`**：Provider 连接信息和协议声明，通过 `offers` 引用 `models` 中的 slug
- **顶层 `routes`**：客户端模型别名到 `provider + model` 的映射
- **顶层 `defaults`**：默认模型、默认 max_tokens、系统提示词

API 请求中可直接使用 `provider/model` 或 `model(provider)` 格式指定模型（如 `deepseek/deepseek-v4-pro`），无需预先定义 route。例如客户端请求 `model: "moonbridge"` 时，会发往 `deepseek` Provider 的 `deepseek-v4-pro`；请求 `model: "gpt-image"` 时，会按 OpenAI Responses 协议直接发往 `openai` Provider 的 `gpt-image-1.5`：
...
`base_url` 应填写 Moon Bridge 追加路径前的协议 base：Anthropic-routed Provider 会请求 `<base_url>/v1/messages`，例如 DeepSeek Anthropic API 使用 `https://api.deepseek.com/anthropic`，Anthropic 官方使用 `https://api.anthropic.com`；`protocol: "openai-response"` Provider 会请求 `<base_url>/v1/responses`，例如 OpenAI 官方使用 `https://api.openai.com`。

旧配置可用迁移脚本更新到当前 v5 结构：
...
`providers.<key>.offers[].pricing` 是可选的 per-offer 价格配置，单位是元（¥）/ M tokens。当某个 offer 配置了价格后，Moon Bridge 会按 session 累加费用，并在每次请求和服务退出时输出费用统计。价格定义在 offer 层，通过 `routes` 关联到别名后自动生效。
...
Web search 支持按 provider 独立配置。在 `providers.<key>.web_search.support` 中为每个 provider 单独设置；顶层 `web_search.support` 作为未在 provider 级别配置时的回退默认值。
...
Kimi 示例使用 Anthropic-compatible base URL；如果你的账号或区域仍使用其他 Moonshot 域名，请保持 `/anthropic` 这一类 Anthropic base 格式，确保最终请求路径是 `/anthropic/v1/messages`。
...
在具体 Provider 模型或 route 下设置 `extensions.deepseek_v4.enabled: true` 可启用 DeepSeek V4 专用兼容逻辑，包括 `reasoning_content` 剥离、thinking 回放、推理输出展示等。推理档位使用与 Codex 兼容的 `default_reasoning_level` / `supported_reasoning_levels` 元数据表达；Transform 会把请求里的 `reasoning.effort` 映射到 DeepSeek Anthropic 兼容参数 `output_config.effort`，其中 `xhigh` 会映射为 DeepSeek 的 `max`。流式 signature-only thinking 会编码进 Codex 可回放的 `reasoning.summary`，旧历史缺少 reasoning/缓存时只在请求侧补空 `thinking` block，不生成空 summary。
...
打开 `trace.enabled: true` 后，Transform 的请求（包括 Anthropic 转换和 OpenAI Responses 直通）和 Capture 模式的代理流量会按模式写入 `data/trace/` 目录，方便排查问题。API Key 等敏感 Header 会自动脱敏。Transform 模式下 trace 按模型建立子目录，例如 `data/trace/Transform/{session_id}/{model}/Response/{n}.json` 和 `data/trace/Transform/{session_id}/{model}/Anthropic/{n}.json`。
...
- **`models`**：共享模型元数据（context_window、extensions、reasoning 等级等），以 slug 为 key
- **`providers`**：Provider 连接信息和协议声明，通过 `offers` 引用模型 slug
- **`routes`**：客户端模型别名到 `provider + model` 的映射
- **`defaults`**：默认模型、默认 max_tokens、系统提示词

OfferEntry 结构如下：

| 字段 | 说明 |
|------|------|
| `model` | 引用顶层 `models` 中的 slug |
| `upstream_name` | 可选，上游真实模型名（为空时与 slug 相同） |
| `priority` | 路由优先级，数值越小优先级越高（0 为最高） |
| `pricing` | 该 provider 对该模型的定价 |
| `overrides` | 可选，覆盖顶层模型元数据 |

动态模型路由遵循三级解析优先级 + fallback 候选链：

1. **Route alias**（最高优先级）：查找 `routes` 表中的精确匹配
2. **Direct ref**：`provider/model` 或 `model(provider)` 格式直接引用
3. **Dynamic reverse index**（最低优先级）：按模型名查找所有提供该模型的 provider，按 offer priority 排序，形成候选链；第一个可用 provider 为 preferred
...
- 顶层 `models` 中的模型定义和 `providers` 中的 `offers` 决定了 `GET /v1/models` 的返回内容
...
生成的 Codex `config.toml` 包含模型提供商信息。需要静态模型目录时，可用 `-print-codex-config` 搭配 `-codex-home` 写入 `models_catalog.json`。
...
| 选项 | 说明 |
|------|------|
| `-config 路径` | 配置文件路径；未指定时读取 `${XDG_CONFIG_HOME:-$HOME/.config}/moonbridge/config.yml` |
| `-addr 地址` | 覆盖监听地址 |
...
| `-dump-config-schema` | 在配置文件旁生成 `config.schema.json` 并退出 |
...
兼容 OpenAI Responses API。支持流式和非流式；无 `/v1` 的 `POST /responses` 也会路由到同一个处理器。

如果配置了 `server.auth_token`，所有 HTTP 请求都必须带 `Authorization: Bearer <token>`；未配置时不做本地认证，上游认证仍由各 Provider 的 `api_key` 处理。
...
返回可用模型列表，格式兼容 Codex CLI 的模型发现接口；无 `/v1` 的 `GET /models` 也可用。
...
设置 `trace.enabled: true` 后，每次请求的完整数据会写入磁盘：

```mermaid
flowchart LR
  subgraph trace["data/trace/"]
    direction TB
    transform["Transform/"]
    resp["Response/\n000001.json\n000002.json\n..."]
    anth["Anthropic/\n000001.json\n000002.json\n..."]
...
注入式 Web Search 由 `internal/extension/websearch` 和 `internal/extension/websearchinjected` 提供工具与 Provider 包装逻辑；当前是在 server/bridge 路径按模型的 resolved web search mode 动态启用，不作为 `BuiltinExtensions()` 注册的独立插件初始化。

### 开发中能力（dev 分支）

以下能力来自 dev 分支，当前文档只作为开发中事实记录，不视为稳定公开接口：

| 能力 | 说明 |
|------|------|
| `foundation/db` | 持久化底座，提供 DB Provider / DB Consumer / Store 抽象 |
| `db_sqlite` | 本地 SQLite 持久化 Provider |
| `db_d1` | Cloudflare D1 持久化 Provider，依赖 Worker 入口注入数据库 |
| `metrics` | 请求指标持久化；绑定数据库后可注册 `GET /v1/admin/metrics` 查询接口 |
| `cmd/cloudflare` | Cloudflare Worker 入口 |
...
Moon Bridge 提供一套完整的 RESTful 管理 API，共 **38 个端点**，路径前缀为 `/api/v1`。通过管理 API 可以可视化地管理 Provider、Model、Route，暂存并应用配置变更。

### 端点概览

| 分组 | 端点数 | 说明 |
|------|--------|------|
| Provider | 6 | CRUD + 连接测试 |
| Offer | 3 | Provider 模型报价管理 |
| Model | 4 | 模型元数据定义 |
| Route | 4 | 路由别名管理 |
| Settings | 7 | 默认值、Web Search、Extensions |
| Config | 4 | 配置导入/导出/校验 |
| Changes | 3 | 暂存变更的列出/应用/丢弃 |
| Status / Stats / Logs / Version | 7 | 系统状态与统计 |

### ConfigStore 持久化

所有管理 API 的变更操作（create / update / delete）不会直接修改运行时配置，而是写入 ConfigStore 的变更表（staged change）。变更经过校验后，通过 `POST /changes/apply` 事务性应用，应用过程中失败会自动回滚。

ConfigStore 默认使用 SQLite 后端，由 `extensions.db_sqlite` 提供。配置变更按以下流程流转：

1. API 请求写入 staged change 表
2. 校验配置有效性
3. `apply` 时构建新的 ProviderManager 验证连接性
4. 原子替换 Runtime snapshot
5. 变更标记为已应用

### Staged Change

每个 pending change 包含 `action`（CREATE / UPDATE / DELETE）、`resource` 类型、`target_key`、`before`（变更前 JSON 快照）和 `after`（变更后 JSON 快照）。应用失败时保留 error 信息，支持批量丢弃。

详细文档见 [docs/api.md](docs/api.md#管理-api-v1)。
