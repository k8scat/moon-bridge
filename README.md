...
打开 `trace_requests: true` 后，Transform 的请求（包括 Anthropic 转换和 OpenAI Responses 直通）和 Capture 模式的代理流量会按模式写入 `data/trace/` 目录，方便排查问题。API Key 等敏感 Header 会自动脱敏。Transform 模式下 trace 按模型建立子目录，例如 `data/trace/Transform/{session_id}/{model}/Response/{n}.json` 和 `data/trace/Transform/{session_id}/{model}/Anthropic/{n}.json`。
